...
手動録画中はGUIの「進行状況」に、取得情報の解析、録画、結合の段階をステップ表示します。取得できる場合は進捗バー、fragment数・バイト数・速度、保存済みファイル数を表示します。**開始からの経過時間**もリアルタイム表示し、終了後は確定した**所要時間**を残します。自動録画一覧にはチャンネルごとの「状態」「進捗」「時間」を表示し、`63%`、`120/300 frag`、速度など、その状態の中でどこまで進んだかも確認できます。
...
高速回線・NVMe・十分なCPU余力がある環境向けに、GUIから `64 / 96 / 128` fragment並列とprefetch倍率 `5〜8` を選べます。
